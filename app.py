            ...
            if request.args.get('per_page') and request.args.get('page') and request.args.get('artist'):
                ...
                if page == 1:
                    data = cursor.execute("""SELECT tracks.name FROM tracks
                                    ...
                                    WHERE artists.name = ? ORDER BY tracks.name LIMIT ? OFFSET ? COLLATE NOCASE""", (artist,per_page, per_page+1)).fetchall()
                else:
                    data = cursor.execute("""SELECT tracks.name FROM tracks
                                    JOIN albums ON tracks.albumid = albums.albumid
                                    JOIN artists ON albums.artistid = artists.artistid 
                                    WHERE artists.name = ? ORDER BY tracks.name LIMIT ? OFFSET ? COLLATE NOCASE""", (artist,per_page, ((page-1)*per_page+1))).fetchall()
