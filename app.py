    ...
    if request.method == 'GET':
            if request.args.get('per_page') and request.args.get('page') and request.args.get('artist'):
                ...
                data = cursor.execute("""SELECT tracks.name FROM tracks
                                    ...
                                    WHERE artists.name = ? ORDER BY tracks.name LIMIT ? OFFSET ? COLLATE NOCASE""", (artist,per_page, page*per_page)).fetchall()
                ...
                data = cursor.execute("""SELECT name FROM tracks
                                    ORDER BY name LIMIT ? OFFSET ? COLLATE NOCASE""", (per_page, page*per_page)).fetchall()
            else:
